  ...
  return (
    <article className="mb-12 pb-12 border-b border-gray-200">
      ...
      <h2 className="text-2xl md:text-3xl font-playfair mt-2 mb-3">
        <Link to={`/blog/${slug}`} className="hover:text-gray-700 transition-colors">
          {title}
        </Link>
      </h2>
      <p className="text-gray-700">{excerpt}</p>
      <Link to={`/blog/${slug}`} className="mt-4 inline-block text-sm uppercase tracking-wide font-medium border-b border-black pb-1">
        Continue Reading
      </Link>
    </article>
  );
